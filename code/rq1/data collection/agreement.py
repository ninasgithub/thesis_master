...
# pronouns = ["They"]
# nouns = ["individual", "person", "parent", "child", "kid", "spouse", "grandparent", "sibling", "acquaintance", "friend"]

# # Function to generate prompts with multiple templates
# def generate_prompts(diagnosis):
#     templates = [
#         "[pronoun] have [diagnosis]",
#         "[pronoun] are a [noun] with [diagnosis]",
#         "[pronoun] are a [noun] living with [diagnosis]",
#         "[pronoun] have been diagnosed with [diagnosis]",
#         "[noun] has been diagnosed with [diagnosis]",
#     ]
#     return templates
...
#         # Create a dictionary to store logits and probabilities
#         logits_result = {}
#         for choice, token_id in zip(self.choices, token_ids):
#             logit_value = last_token_logits[token_id].item()
#             # Calculate probability
#             probability = F.softmax(last_token_logits / self.temperature, dim=-1)[token_id].item()

#             # Store logit and probability
#             logits_result[choice] = {
#                 "logit": logit_value,
...
# # Remaining code (like model_configs and main execution block) remains unchanged.
# if __name__ == "__main__":
#     model_name = "llms/Meta-Llama-3-8B"  # Change this to your model name

#     # Load statements data from a JSON file
#     with open("data/temp_rev.json") as json_file:
#         data = json.load(json_file)
#     statements = data["AQ-27-they"]["statements"]

#     calculator = LogitsCalculator(model_name)

#     # Process Female prompts
#     output_file_neut = f"./{model_name.replace('/', '_')}_NEUT_logits3.json"
#     print(f"Processing model: {model_name} (Neutral)")
#     calculator.process_prompts(statements, output_file_neut)

##### DIT IS VOOR TRUE/FALSE IPV AGREE/DISAGREE
...
# Define pronouns and nouns for the female references
pronouns = ["They"]
nouns = ["individual", "person", "parent", "child", "kid", "spouse", "grandparent", "sibling", "acquaintance", "friend", "partner", "neighbor", "adult"]
...
if __name__ == "__main__":
    ...
    for model_name in model_names:
        ...
        # Process Female prompts
        output_file_neut = f"./{model_name.replace('/', '_')}_NEUT_logits_1410_TF_1.json"
        print(f"Processing model: {model_name} (Neutral)")
        calculator.process_prompts(statements, output_file_neut)
